...
inline bool launches(Model &m) {
  ...
  for (JsonObjectConst r : results) {
    ...
    detFilter["mission"]["orbit"]["abbrev"] = true;
    detFilter["rocket"]["launcher_stage"][0]["landing"]["attempt"] = true;
    const char *id = r["id"] | "";
    // The booster and the orbit do not change between wakes, so a launch we
    ...
    if (haveCached) {
      ...
      orbit   = det["mission"]["orbit"]["abbrev"] | "";
      attempt = landing["attempt"] | false;
    }

    // Which way the track leans, from the orbit the mission is going to.
    //
