...
Two constraints pull in opposite directions, so the reader switches mode at the
16 MB line:

  below 0x1000000  the ROM loader (`--no-stub`) reads reliably, where the stub
                   flasher corrupts roughly three reads in five
  at and above     the ROM loader refuses outright -- "Can't access flash regions
                   larger than 16MB" -- and only the stub flasher can reach
...
ROM_ADDRESS_LIMIT = 0x1000000  # the ROM loader cannot reach flash at or above 16 MB
...
def main():
    ...
    ap.add_argument("--stub-mode", choices=("auto", "stub", "no-stub"), default="auto",
                    help="auto (default) uses the ROM loader below 16 MB and the stub flasher at or "
                         "above it; stub or no-stub forces one mode for every chunk")
    ...
    for i in range(total):
        ...
        if os.path.exists(path) and os.path.getsize(path) == size:
            continue
        if a.stub_mode == "auto":
            no_stub = offset + size <= ROM_ADDRESS_LIMIT
        else:
            no_stub = a.stub_mode == "no-stub"
        ok, info = read_chunk(a.esptool, a.port, a.baud, a.chip, offset, size, path, a.retries, no_stub)
        done = offset + size
        elapsed = time.time() - started
        rate = done / elapsed if elapsed else 0
        if ok:
            print("chunk %3d/%d  0x%08x  %s  %-7s %5.1f%%  %s attempt(s)  eta %s" % (
                i + 1, total, offset, human(size), "rom" if no_stub else "stub",
                100 * done / a.size, info,
