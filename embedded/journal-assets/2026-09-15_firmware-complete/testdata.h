...
inline void loadTestFrame(Model &m, int mode) {
  ...
  switch (mode) {
    // Newest first: the white-R button steps 1, 2, 3... so whatever is being
    // worked on should be one press away, not five.
    case 1:   // the quiet day — no launch, so the band carries the line of the day
      m.launchTonight = false;
      m.quoteIdx = 2;                        // the longest one, which drops a size
      break;

    case 2:   // the water filter badge on its first day, green
      m.launchTonight = false;
      m.quoteIdx = 0;
      m.filterDay = 0;
      break;

    case 3:   // every band at once — the tightest layout there is
      m.filterDay = 2;                       // the red day, beside a full footer
    ...
    case 4:   // wildfire smoke and a flat battery
    ...
    case 5:   // carried-over data, nothing fetched
      m.stale = true; m.missedRefreshes = 3;
      m.aqi = -1;
      m.batteryPct = 44; m.runwayDays = 44;
      break;

    case 6:   // a wet week, moderate air, longest possible event name
