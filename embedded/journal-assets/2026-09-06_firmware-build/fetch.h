...
inline bool httpGetJson(const char *host, const char *path, JsonDocument &doc, size_t filterHint = 0) {
...
// see it and lift-off falls after sunset or before sunrise.
inline bool skyIsClear(const Model &m) {
  return m.nowCode <= 2;                 // WMO 0-2: clear to partly cloudy
}

inline bool launches(Model &m) {
  m.launchTonight = false;
  if (!skyIsClear(m)) return true;       // nothing to show, but not a failure

  JsonDocument doc;
  if (!httpGetJson(LL2_HOST, LL2_PATH, doc)) return false;
  ...
  for (JsonObjectConst r : results) {
    ...
    snprintf(m.launchName, sizeof(m.launchName), "%s", r["name"] | "LAUNCH");
    for (char *p = m.launchName; *p; p++) *p = toupper(*p);

    const char *landing = r["rocket"]["launcher_stage"][0]["landing"]["location"]["abbrev"] | "";
    if (strstr(landing, LANDING_PAD_HINT))
      snprintf(m.launchSub, sizeof(m.launchSub), "%s - 200 mi - booster to %s - booms", LAUNCH_LOOK_LABEL, landing);
    else
