// Colour and type calibration for the reTerminal E1002 (Spectra 6).
//
// The dashboard design assumes six colours that read distinctly, and that type
// stays legible on each of the coloured bands. Screen previews flatter e-paper,
...
// and GxEPD_* names in the colour list. Running this sketch settles which the
...
struct Swatch { uint16_t colour; const char* name; };
Swatch SWATCHES[] = {
  { C_BLACK,  "BLACK"  },
  { C_WHITE,  "WHITE"  },
  { C_RED,    "RED"    },
...
void setup() {
  ...
  // ── Row 1: the six colours as solid blocks, 800/6 wide ──────────────────
  const int sw = 800 / N_SWATCH, sh = 150;
  for (int i = 0; i < N_SWATCH; i++) {
    epaper.fillRect(i * sw, 0, sw, sh, SWATCHES[i].colour);
    epaper.drawRect(i * sw, 0, sw, sh, C_BLACK);
    epaper.setTextColor(SWATCHES[i].colour == C_BLACK ? C_WHITE : C_BLACK);
    epaper.setTextFont(2);
    epaper.drawString(SWATCHES[i].name, i * sw + 8, sh - 24);
  }

  // ── Row 2: each colour as a thin rule, to see what survives at 2 px ──────
  int y = sh + 14;
  for (int i = 0; i < N_SWATCH; i++) {
    epaper.fillRect(20, y + i * 9, 360, 2, SWATCHES[i].colour);
  }
  ...
  // ── Row 4: the dashboard's own colour pairings ───────────────────────────
  int py = 300, ph = 28;
