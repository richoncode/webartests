...
public:
  ...
  bool ensureConnected(uint32_t timeoutMs = 1200) {
  ...
  // Big enough for a scene: three units is 80 hex characters, and the quotes
  // pushed that past an 80-byte buffer, so the bulb got an unterminated
  // string and never answered.
  bool setStr(int dp, const char *v) { char b[192]; snprintf(b, sizeof(b), "\"%s\"", v); return setRaw(dp, b); }
...
private:
  ...
  bool exchange(uint32_t cmd, const char *json, JsonDocument &out) {
    ...
    // Drop anything still sitting in the socket. The connection is persistent
    // and a write's acknowledgement arrives on it too, so a query issued right
    // after a write can read the write's reply and see no datapoints at all.
    while (_sock.available()) _sock.read();
