...
inline bool launches(Model &m) {
  ...
  // The detailed response runs to ~83 kB. Keep only the fields drawn, so the
  // document on the heap is a few hundred bytes rather than the whole payload.
  JsonDocument filter;
  filter["results"][0]["net"] = true;
  filter["results"][0]["name"] = true;
  filter["results"][0]["rocket"]["launcher_stage"][0]["landing"]["location"]["abbrev"] = true;
  filter["results"][0]["rocket"]["launcher_stage"][0]["landing"]["type"]["abbrev"] = true;
  ...
  for (JsonObjectConst r : results) {
    ...
    JsonObjectConst landing = r["rocket"]["launcher_stage"][0]["landing"];
    const char *where = landing["location"]["abbrev"] | "";
    const char *how   = landing["type"]["abbrev"] | "";
