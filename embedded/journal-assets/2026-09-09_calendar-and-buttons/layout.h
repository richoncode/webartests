...
// When the event and bins bands both appear they sit side by side on one row,
// because stacking all three bands leaves the forecast strip at 73 px against
// the 104 px its contents need.
static const int16_t HALF_W    = 398;
...
inline Layout computeLayout(bool showEvent, bool showBins, bool showLaunch) {
  Layout L{};
  L.showEvent = showEvent;
  L.showBins = showBins;
  L.showLaunch = showLaunch;
  L.sharedRow = showEvent && showBins;
  ...
  if (L.sharedRow) {
    ...
    L.bins       = { (int16_t)(HALF_W + RULE), y, HALF_W, EVT_H };
    bb -= EVT_H + RULE;
    L.bandRule = { 0, bb, PANEL_W, RULE };
  } else if (showEvent) {
    L.event = { 0, (int16_t)(bb - EVT_H), PANEL_W, EVT_H };
    bb -= EVT_H + RULE;
    L.bandRule = { 0, bb, PANEL_W, RULE };
  } else if (showBins) {
