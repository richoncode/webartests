...
inline bool launches(Model &m) {
  ...
  for (JsonObjectConst r : results) {
    ...
    // AM/PM sits with the time rather than on the line below: "PM - 45 min
    // after sunset" is 24 characters and, right-aligned, reached back into the
    // PRIME slot.
    ...
    if (after <= LAUNCH_PRIME_WINDOW_MIN)
      snprintf(m.launchTz, sizeof(m.launchTz), "%d min after sunset", after);
    else
      snprintf(m.launchTz, sizeof(m.launchTz), "well after dark");
